...
import jsonld from 'metalsmith-jsonld';

export default (siteURL, options) => {
    return (files, metalsmith, done) => {
        ...
        Object.keys(files)
            .forEach((filename) => {
                ...
                // Fix https://github.com/thinktandem/metalsmith-jsonld/issues/3
        ...
        jsonld(options)(files, metalsmith, (...args) => {
            ...
            Object.keys(files)
                .forEach((filename) => {
                    // Fix https://github.com/thinktandem/metalsmith-jsonld/issues/2
                    if (files[filename].tags && !Array.isArray(files[filename].tags)) {
                        files[filename].tags = files[filename].tags.split(',')
                    }

                    // Make the output of metalsmith-jsonld actually usable...
